(function($){
	...
	var do_get_visual = function (co) {
		return co.replace(/\[html5audio([^\]]*)\]/g, function(a,b){
			var html = '<img src="'+tinymce.baseURL+'/plugins/wpgallery/img/t.gif" class="html5audio mceItem" title="'+tinymce.DOM.encode(a)+'" />';						
			return html;
		});
	};
	...
	var _createButtons = function (url) {
		...
		DOM.remove('html5audio_button');
		
		DOM.add(document.body, 'div', {
			id : 'html5audio_button',
			style : 'display:none;'
		});
		
		editButton = DOM.add('html5audio_button', 'img', {
		...
		tinymce.dom.Event.bind(editButton, 'mousedown', function(e) {
			...
			if ( el.nodeName == 'IMG' && ed.dom.hasClass(el, 'html5audio') ) {
				shortcode.buildResponse(ed, url, d);
			}
		});
		
		dellButton = DOM.add('html5audio_button', 'img', {
			src : url+'/images/delete.png',
			id : 'html5audio_delete',
			width : '24',
			height : '24',
			title : 'Delete Quote'
		});
	...
	var _hideButtons = function () {
		...
		if ( document.getElementById('html5audio_button') )
			tinymce.DOM.hide('html5audio_button');
	...
	var doinit = function (ed, url, op) {
		...
		ed.onMouseDown.add(function(ed, e) {
			if ( e.target.nodeName == 'IMG' && ed.dom.hasClass(e.target, 'html5audio') )
				_showButtons(e.target, 'html5audio_button');
		});
	};
	...
	var shortcode = $('html').jshortcode({
			...
			fields						: [ {type : 'select', name : 'Select Video Size '	, id: 'size' , option : ['span12', 'span11', 'span10', 'span9', 'span8', 'span7', 'span6', 'span5', 'span4', 'span3', 'span2', 'span1',]} ,		      						    
			      						    {type : 'upload'  , name : 'MP3 File'     		, id: 'mp3'} , 
		...
		function(ed, url, options) 
		{
			...
			var mp3			= $('#'+options.pluginprefix + 'mp3').val();
			var videoimage  = $('#'+options.pluginprefix + 'image').val();
			var clearbottom = $('#'+options.pluginprefix + 'clearbottom').is(':checked');
			
			var html = '[html5audio size="' + videosize + '" mp3="' + mp3 + '" image="' + videoimage + '" clearbottom="'+ clearbottom +'"]';
