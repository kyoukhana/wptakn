(function($){
	...
	var do_get_visual = function (co) {
		return co.replace(/\[youtube([^\]]*)\]/g, function(a,b){
			var html = '<img src="'+tinymce.baseURL+'/plugins/wpgallery/img/t.gif" class="youtube mceItem" title="'+tinymce.DOM.encode(a)+'" />';						
			return html;
		});
	};
	...
	var _createButtons = function (url) {
		...
		editButton = DOM.add('youtube_button', 'img', {
		...
		tinymce.dom.Event.bind(editButton, 'mousedown', function(e) {
			...
			if ( el.nodeName == 'IMG' && ed.dom.hasClass(el, 'youtube') ) {
				shortcode.buildResponse(ed, url, d);
			}
		});
		
		dellButton = DOM.add('youtube_button', 'img', {
	...
	var _hideButtons = function () {
		...
		if ( document.getElementById('youtube_button') )
			tinymce.DOM.hide('youtube_button');
	...
	var doinit = function (ed, url, op) {
		...
		ed.onMouseDown.add(function(ed, e) {
			if ( e.target.nodeName == 'IMG' && ed.dom.hasClass(e.target, 'youtube') )
				_showButtons(e.target, 'youtube_button');
		});
	};
	...
	var shortcode = $('html').jshortcode({
		...
		function(ed, url, options) 
		{
			...
			var html = '[youtube src="' + videourl + '" size="' + videosize + '" clearbottom="'+ clearbottom +'"]';
